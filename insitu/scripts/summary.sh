...
pegasus-statistics -s all ${INST_WORK_HOME}/scripts/do7/pegasus/ascent-test-workflow/run0001/ >/dev/null 2>&1
cp -r ${INST_WORK_HOME}/scripts/do7/pegasus/ascent-test-workflow/run0001/ ${path}/simulation 
...
echo "************************SUMMARY************************" 
echo "simulation"
cat ${path}/simulation/statistics/jobs.txt | grep ascent_executable_ID
...
start=$(head -n 1 ${path}/simulation/ascent-test-workflow-0.log | cut -d " " -f 4)
